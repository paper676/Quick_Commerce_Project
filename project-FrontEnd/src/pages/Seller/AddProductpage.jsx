...
function AddProductpage() {
    ...
    const onSubmitHandler = async (event) => {
        try {
            ...
            //conver productData into json-format
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                <img className='max-w-24 cursor-pointer' width={100} height={100} src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area} alt="UploadArea" />
                            </label>
                        ))}
                    </div>
                </div>
